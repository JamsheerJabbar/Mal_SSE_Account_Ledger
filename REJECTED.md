...
**What would change my mind:** a demonstrated case where an intervening overdraft fee or reversal, between approval and settlement, pushes the account further negative than the fee structure assumes when the blind settlement is let through - that would argue for a bounded re-check rather than none at all. `KnownLimitationsTest.settlementIsNotReRefusedAfterAnInterveningFee` is exactly that case, now built and left deliberately failing: a 100 AED hold approved at zero headroom settles in full against a balance an intervening fee has already dropped to -125, and the settlement itself then earns a second fee. It's closer to a case for changing this than not - see `ARCHITECTURE.md` §5 for why it's tracked as an open, red test rather than silently fixed.
